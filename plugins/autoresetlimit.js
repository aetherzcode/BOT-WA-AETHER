...
let handler = async (m, { conn, args, command }) => {
    ...
    if (args.length === 0) {
        ...
        return conn.reply(
            m.chat,
            `*'on' atau 'off'!*\n\nContoh:\n- *.${command} on* untuk mengaktifkan reset otomatis setiap jam 11:20\n- *.${command} off* untuk menonaktifkan reset otomatis`,
            null
        );
    }
    ...
    if (args[0] === 'on') {
        ...
        conn.reply(m.chat, `*Reset limit otomatis akan dijalankan setiap jam 11:20.*`, null);
    } else if (args[0] === 'off') {
        if (!isAutoResetEnabled) {
...
function resetLimit(conn, lim) {
    ...
    conn.reply('120363377709205706@g.us', `*Limit berhasil direset ${lim} / user*`, null); // Kirim info ke grup tertentu
}

function getTimeUntilMidnight() {
    let now = new Date();
    let nextMidnight = new Date(now);
    nextMidnight.setHours(11, 20, 0, 0); 
