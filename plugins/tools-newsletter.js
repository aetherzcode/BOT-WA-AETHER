let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (option === '--info') {
    ...
    let aether = await conn.newsletterMetadata("invite", code)
    let hasil = `
ID: ${aether.id}
State: ${aether.state}
Creation Time: ${new Date(aether.creation_time * 1000).toLocaleString()}
Name: ${aether.name}
Description: ${aether.description}
Invite: ${aether.invite}
Subscribers: ${aether.subscribers}
Verification: ${aether.verification}
`
    await m.reply(hasil)
    return
  }
