...
let handler = async (m, {
    ...
    //Manage panel
    const domain = `https://aether.pannel-private.me` // Domain
    const apikey = `ptla_SZAYcTwx9ysb5R1m3hvXAG2ktjxD3qlWoHZLJceITYl` // Apikey
    const c_apikey = `ptlc_jZAUwbFaxRDn5Zu8jovsUOWrFo5dEMqylu4xjcv3ant` // c Apikey

    const webPage = `https://aether.pannel-private.me` // Web Page
...
${_p + command} user,nomor`)
	...
	let cpu = "30"
	let disk = "1024"
	let email = username + "@@aetherz.xyz"
...
m.reply(`BERHASIL DIBUAT OLEH AETHERZBOTZ

*BERHASIL MEMBUAT USER + SERVER ID:* ${user.id}`)
...
2. JANGAN MEMBAGIKAN AKUN PANEL ANDA
3. DILARANG MEMBAGIKAN WEBSITE PANEL
4. DILARANG MEMAKSA REFF
5. JANGAN LUPA MENGUCAPKAN TERIMA KASIH
	`
	let msgs = generateWAMessageFromContent(u, {
        viewOnceMessage: {
            message: {
                "messageContextInfo": {
                    "deviceListMetadata": {},
                ...
                interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                        buttons: [{
                 "name": "cta_copy",
                 "buttonParamsJson": `{\"display_text\":\"Cᴏpʏ ᴜsᴇʀɴᴀᴍᴇ 📝\",\"id\":\"123456789\",\"copy_code\":\"${user.username}\"}`
              },
              {
                 "name": "cta_copy",
                 "buttonParamsJson": `{\"display_text\":\"Cᴏpʏ ᴘᴀssᴡᴏʀᴅ 🔐\",\"id\":\"123456789\",\"copy_code\":\"${password}\"}`
              }, 
              {
        "name": "cta_url",
        "buttonParamsJson": `{\"display_text\":\"Lᴏɢɪɴ 🌐\",\"url\":\"${domain}\",\"merchant_url\":\"https://digistore.@aetherz.xyz\"}`
 }],
          }), 
          contextInfo: {
    ...
    await conn.relayMessage(u, msgs.message, {
        messageId: m.key.id
    })
  ...
  let server = res.attributes
  let p = await m.reply(`
*BERHASIL MENAMBAHKAN USER + SERVER*
TIPE: pengguna
ID: ${user.id}
NAMA: ${user.first_name} ${user.last_name}
MEMORI: ${server.limits.memory === 0 ? 'Tidak Terbatas' : server.limits.memory} MB
PENYIMPANAN: ${server.limits.disk === 0 ? 'Tidak Terbatas' : server.limits.disk} MB
            ...
            message: {
                ...
                interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                        buttons: [{
                "name": "quick_reply",
                "buttonParamsJson": `{\"display_text\":\"Hᴀʟᴀᴍᴀɴ sᴇʟᴀɴᴊᴜᴛɴʏᴀ\",\"id\":\"${_p}listusr ${res.meta.pagination.current_page + 1}\"}`
              }],
                    })
                })
            }
...
break;
        case "delusr": {
            ...
            if (res.errors) return m.reply("*Pengguna Tidak Terdaftar*")
            m.reply("*Berhasil Menghapus Pengguna*")
        }
        break
        case "detusr": {
            let usr = args[0]
            ...
            message: {
                ...
                interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                        buttons: [{
                "name": "quick_reply",
                "buttonParamsJson": `{\"display_text\":\"Hᴀʟᴀᴍᴀɴ sᴇʟᴀɴᴊᴜᴛɴʏᴀ️\",\"id\":\"${_p}listsrv ${res.meta.pagination.current_page + 1}\"}`
              }],
                    })
                })
            }
